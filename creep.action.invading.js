const action = new Creep.Action('invading');
...
action.getFlaggedStructure = function(flagColor, pos){
    const flagsEntries = FlagDir.filter(flagColor, pos, true);
    const target = [];
    let checkFlag = flagEntry => {
        ...
        if( flag && flag.pos.roomName === pos.roomName && flag.room !== undefined ){ // room is visible
            const targets = flag.room.lookForAt(LOOK_STRUCTURES, flag.pos.x, flag.pos.y);
...
action.newTarget = function(creep){
    ...
    const flag = FlagDir.find(FLAG_COLOR.invade, creep.pos);
