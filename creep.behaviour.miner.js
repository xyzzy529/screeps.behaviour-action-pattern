...
mod.determineTarget = creep => {
	...
	if (SAY_ASSIGNMENT)
		creep.say(String.fromCharCode(9935), SAY_PUBLIC);
};
mod.run = function (creep, params = {}) {
	if (_.isUndefined(params.approach))
		params.approach = mod.approach;
	if (_.isUndefined(params.determineTarget))
		params.determineTarget = mod.determineTarget;
	let source;
	...
	if (source) {
		if (!creep.action || creep.action.name !== 'harvesting')
		...
		if (!creep.data.determinatedSpot) {
			...
			const containerSpot = (source.container && source.container.pos.isNearTo(source)
				 && !_.some(invalid, {
					x : source.container.pos.x,
					y : source.container.pos.y
				})) ? source.container.pos : null;
			let spots = [];
			let args;
			if (!containerSpot) {
				args = {
			...
			if (containerSpot || spots.length > 0) {
				...
				if (spot) {
					...
					if (!params.remote) {
						...
						if (spawn) {
							let path = spot.findPathTo(spawn, {
									ignoreCreeps : true
								});
							if (path)
								creep.data.predictedRenewal = creep.data.spawningTime + path.length; // road assumed
						}
					}
		...
		if (creep.data.determinatedSpot) {
			...
			if (source.energy === 0) { // for mineral miners source.energy is undefined so this is false
				...
				if (!creep.data.repairChecked || Game.time - creep.data.repairChecked > MINER_WORK_THRESHOLD) {
					...
					if (repairTarget) {
						if (CHATTY)
							creep.say('repairing', SAY_PUBLIC);
				...
				if (CHATTY)
					creep.say('harvesting', SAY_PUBLIC);
				if (range === 0) {
				...
				if (range === 0) {
					if (creep.sum > (creep.carryCapacity - perHarvest(creep))) {
						let transfer = r => {
							if (creep.carry[r] > 0)
								creep.transfer(source.container, r);
						};
				...
				if (CHATTY)
					creep.say('dropmining', SAY_PUBLIC);
				if (range === 0) {
					if (creep.sum > (creep.carryCapacity - perHarvest(creep))) {
						if (OOPS)
							creep.say(String.fromCharCode(8681), SAY_PUBLIC);
						let drop = r => {
							if (creep.carry[r] > 0)
								creep.drop(r);
						};
